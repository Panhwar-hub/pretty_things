...
class SiteSettingsController extends Controller
{
    ...
    public function updatewelcomeSlider(Request $request)
    {
      ...
      $request->validate([
        ...
        if(request()->hasFile('welcomeslider')){
            $homeslider = request()->file('welcomeslider')->store('Uploads/welcomeslider/'.$request->id.rand().rand(10,100), 'public');
            $image = imagetable::where('ref_id',$request->id)->where('table_name','welcome-slider')->update (
             [
             'table_name' => 'welcome-slider',
             'img_path' => $homeslider,
             'ref_id' => $request->id,
